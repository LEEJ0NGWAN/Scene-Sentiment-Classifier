...
class Moception(nn.Module):

    def __init__(self, num_classes=1000, aux_logits=True, transform_input=False,
                 inception_blocks=None):
        super(Moception, self).__init__()
        ...
        self.Conv2d_1a_2x3 = conv_block(2, 32, kernel_size=3, stride=2) #lhg
        ...
        stackfc = []
        stackfc.append(nn.Linear(4096, 4096))
        stackfc.append(nn.Linear(4096, 2048))
        self.stackfc = nn.Sequential(*stackfc)
    ...
    def _transform_motion_input(self, x): #LHG
        if self.transform_input:
            x_ch0 = torch.unsqueeze(x[:, 0], 1) * (0.229 / 0.5) + (0.485 - 0.5) / 0.5
            x_ch1 = torch.unsqueeze(x[:, 1], 1) * (0.224 / 0.5) + (0.456 - 0.5) / 0.5
            x = torch.cat((x_ch0, x_ch1), 1)
        return x

    def _forward(self, x, channel=3):
        # N x 3 x 299 x 299
        if channel == 2:
            x = self.Conv2d_1a_2x3(x)
        else:
    ...
    def forward(self, x1, x2, motion_batch):
        ######### LHG : Calc Motion ########
        ...
        for i in range(batch_size):
            ...
            flow_arr[i] = channel
            # Visualizing Training (Generally : False)
            if False:
        ...
        x2 = self._transform_motion_input(motion_batch)
        x2, aux = self._forward(x2, channel=2) # 2048
        x = self.stackfc(torch.cat([x1, x2], 1)) # 4096 -> 2048
        x = self.fc(x) # 2048 -> classnum

        aux_defined = False #self.training and self.aux_logits
        if torch.jit.is_scripting():
            if not aux_defined:
                warnings.warn("Scripted Moception always returns Moception Tuple")
            return InceptionOutputs(x, aux)
        else:
